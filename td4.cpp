...
// Utilisation ds structures et classes créées
...
int main()
{
	...
	Film* hobbitFilmp = dynamic_cast<Film*>(trouver(bibliotheque, [](const auto& f) { return f.titre == "Le Hobbit : La Bataille des Cinq Armées"; }));
	Livre* hobbitLivrep = dynamic_cast<Livre*>(trouver(bibliotheque, [](const auto& f) { return f.titre == "The Hobbit"; }));

	FilmLivre hobbit(hobbitFilmp, hobbitLivrep);
	...
	Item* quelconque = trouver(bibliotheque, [](const auto& f) { return f.titre == "Inception"; }); //trouver un item qui nexiste pas (retourne nullptr)
